...
const simsignalwrap_t
  TraCIMobility::parkingStateChangedSignal = simsignalwrap_t(
    TRACI_SIGNAL_PARKING_CHANGE_NAME);

namespace {
const double MY_INFINITY = (std::numeric_limits<double>::has_infinity ?
...
void TraCIMobility::initialize(int stage) {
  if (stage == 0) {
    ...
    // don't call BaseMobility::initialize(stage) -- our parent will take care
    // to call changePosition later
  } else {
    BaseMobility::initialize(stage);
  }
}
...
void TraCIMobility::nextPosition(const Coord &position,
                                 std::string road_id,
                                 double speed,
                                 double angle,
                                 TraCIScenarioManager::VehicleSignal signals) {
  if (debug)
    EV << "nextPosition " << position.x << " " << position.y << " " << road_id
...
void TraCIMobility::changePosition() {
  ...
  move.setStart(
    Coord(nextPos.x, nextPos.y, move.getCurrentPosition().z)); // keep z
                                                               // position
  move.setDirectionByVector(Coord(cos(angle), -sin(angle)));
  move.setSpeed(speed);
  if (hasGUI())
...
double TraCIMobility::calculateCO2emission(double v, double a) const {
  ...
  // Cappiello, A. and Chabini, I. and Nam, E.K. and Lue, A. and Abou Zeid, M.,
  // "A statistical model of vehicle emissions and fuel consumption," IEEE 5th
  // International Conference on Intelligent Transportation Systems (IEEE ITSC),
  // pp. 801-809, 2002
  ...
  // power in W
  double P_tract = A * v + B * v * v + C * v * v * v
                   + M * a * v; // for sloped roads: +M*g*sin_theta*v
