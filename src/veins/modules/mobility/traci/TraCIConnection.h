...
public:
  ...
  /**
   * sends a single command via TraCI, checks status response, returns
   * additional responses
   */
  TraCIBuffer query(uint8_t commandId, const TraCIBuffer &buf = TraCIBuffer());

  /**
   * sends a single command via TraCI, expects no reply, returns true if
   * successful
   */
  TraCIBuffer queryOptional(uint8_t commandId,
 ...
 */
std::string
makeTraCICommand(uint8_t commandId, const TraCIBuffer &buf = TraCIBuffer());
