...
TraCILauncher::TraCILauncher(std::string commandLine) {
  ...
  if (!bSuccess) {
    ...
    msg = std::string() + "Error launching TraCI server (\"" + commandLine
          + "\"): " + msg + ". Make sure you have set $PATH correctly.";
  ...
  if (pid == 0) {
    ...
    if (r == -1) {
      throw cRuntimeError("Running \"%s\" failed during system()",
                          commandLine.c_str());
    }
    if (WEXITSTATUS(r) != 0) {
      throw cRuntimeError("Error launching TraCI server (\"%s\"): exited with "
                          "code %d. Make sure you have set $PATH correctly.",
                          commandLine.c_str(),
                          WEXITSTATUS(r));
    }
    exit(1);
