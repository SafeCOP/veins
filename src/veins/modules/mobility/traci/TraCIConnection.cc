...
TraCIConnection *TraCIConnection::connect(const char *host, int port) {
  ...
  for (int tries = 1; tries <= 10; ++tries) {
    *socketPtr = ::socket(AF_INET, SOCK_STREAM, 0);
    ...
    std::stringstream ss;
    ss << "Could not connect to TraCI server; error message: " << sock_errno()
       << ": " << strerror(sock_errno());
    ...
    if (tries >= 10) {
      ...
      EV_WARN << msg << " -- Will retry in " << sleepDuration << " second(s)."
              << std::endl;
    }
...
TraCIBuffer TraCIConnection::query(uint8_t commandId, const TraCIBuffer &buf) {
  ...
  if (result == RTYPE_NOTIMPLEMENTED)
    throw cRuntimeError("TraCI server reported command 0x%2x not implemented "
                        "(\"%s\"). Might need newer version.",
                        commandId,
                        description.c_str());
  if (result == RTYPE_ERR)
    throw cRuntimeError("TraCI server reported error executing command 0x%2x "
                        "(\"%s\").",
                        commandId,
                        description.c_str());
  ...
  uint32_t msgLength;
  {
    ...
    while (bytesRead < sizeof(uint32_t)) {
      ...
      if (receivedBytes > 0) {
        ...
        throw cRuntimeError("Connection to TraCI server closed unexpectedly. "
                            "Check your server's log");
      } else {
        if (sock_errno() == EINTR)
          continue;
        if (sock_errno() == EAGAIN)
          continue;
        throw cRuntimeError("Connection to TraCI server lost. Check your "
                            "server's log. Error message: %d: %s",
                            sock_errno(),
                            strerror(sock_errno()));
      }
    }
...
void TraCIConnection::setNetbounds(TraCICoord netbounds1,
...
Coord TraCIConnection::traci2omnet(TraCICoord coord) const {
  return Coord(coord.x - netbounds1.x + margin,
               (netbounds2.y - netbounds1.y) - (coord.y - netbounds1.y)
                 + margin);
}

std::list<Coord>
TraCIConnection::traci2omnet(const std::list<TraCICoord> &list) const {
...
std::list<TraCICoord>
TraCIConnection::omnet2traci(const std::list<Coord> &list) const {
  std::list<TraCICoord> result;
  std::transform(list.begin(),
                 list.end(),
                 std::back_inserter(result),
                 std::bind1st(std::mem_fun<TraCICoord, TraCIConnection, Coord>(
                                &TraCIConnection::omnet2traci),
                              this));
  return result;
}
...
double TraCIConnection::traci2omnetAngle(double angle) const {
  ...
  // rotate angle so 0 is east (in TraCI's angle interpretation 0 is north, 90
  // is east)
  angle = 90 - angle;
...
double TraCIConnection::omnet2traciAngle(double angle) const {
  ...
  // rotate angle so 0 is south (in OMNeT++'s angle interpretation 0 is east, 90
  // is north)
  angle = 90 - angle;
