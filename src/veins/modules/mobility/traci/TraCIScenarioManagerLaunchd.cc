...
void TraCIScenarioManagerLaunchd::initialize(int stage) {
  ...
  if (basedir_nodes.size() == 0) {
    ...
    std::string basedir = cSimulation::getActiveSimulation()
                            ...
                            .getBaseDirectory();
    cXMLElement *basedir_node = new cXMLElement("basedir",
                                                __FILE__,
                                                launchConfig);
...
void TraCIScenarioManagerLaunchd::init_traci() {
  {
    ...
    if (apiVersion == 1) {
      ...
      error("TraCI server \"%s\" reports API version %d, which is unsupported. "
            "We recommend using the version of sumo-launchd that ships with "
            "Veins.",
            serverVersion.c_str(),
            apiVersion);
    }
  ...
  if (commandResp != CMD_FILE_SEND)
    error("Expected response to command %d, but got one for command %d",
          CMD_FILE_SEND,
          commandResp);
  uint8_t result;
  obuf >> result;
  ...
  obuf >> description;
  if (result != RTYPE_OK) {
    EV << "Warning: Received non-OK response from TraCI server to command "
       << CMD_FILE_SEND << ":" << description.c_str() << std::endl;
  }
