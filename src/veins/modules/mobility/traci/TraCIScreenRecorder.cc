...
void TraCIScreenRecorder::initialize(int stage) {
  if (stage == 0) {
    ...
    takeScreenshot->setSchedulingPriority(1); // this schedules screenshots
                                              // after TraCI timesteps
...
void TraCIScreenRecorder::handleMessage(cMessage *msg) {
  ...
  if (std::string(dirname) == "") {
    dirname = cSimulation::getActiveSimulation()
                ->getEnvir()
                ->getConfigEx()
                ->getVariable(CFGVAR_RESULTDIR);
  }

  // get absolute path of dirname (the TraCI server might be running in a
  // different directory than OMNeT++)
...
#ifdef _WIN32
    // replace ':' with '-'
    for (std::string::iterator i = filenameTemplate.begin();
         i != filenameTemplate.end();
         ++i)
      if (*i == ':')
        *i = '-';
#endif /* _WIN32 */
