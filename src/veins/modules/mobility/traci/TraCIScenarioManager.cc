...
TraCIScenarioManager::TypeMapping
TraCIScenarioManager::parseMappings(std::string parameter,
   ...
   * possible syntaxes
   *
   * "a"          : assign module type "a" to all nodes (for backward
   * compatibility) "a=b"        : assign module type "b" to vehicle type "a".
   * the presence of any other vehicle type in the simulation will cause the
   * simulation to stop "a=b c=d"    : assign module type "b" to vehicle type
   * "a" and "d" to "c". the presence of any other vehicle type in the
   * simulation will cause the simulation to stop "a=b c=d *=e": everything
   * which is not of vehicle type "a" or "b", assign module type "e" "a=b c=0"
   * : for vehicle type "c" no module should be instantiated "a=b c=d *=0":
   * everything which is not of vehicle type a or c should not be instantiated
   *
   */
  ...
  for (i = 0; i < typeMappings.size(); i++) {
    ...
    if (mapping.size() == 1) {
      ...
      if (typeMappings.size() != 1)
        ...
        throw cRuntimeError("parameter \"%s\" includes multiple mappings, but "
                            "\"%s\" is not mapped to any vehicle type",
                            parameterName.c_str(),
                            mapping[0].c_str());
      else
      ...
      if (mapping.size() != 2)
        throw cRuntimeError("invalid syntax for mapping \"%s\" for parameter "
                            "\"%s\"",
                            typeMapping.c_str(),
                            parameterName.c_str());
      // check that the mapping does not already exist
      if (map.find(mapping[0]) != map.end())
        throw cRuntimeError("duplicated mapping for vehicle type \"%s\" for "
                            "parameter \"%s\"",
                            mapping[0].c_str(),
                            parameterName.c_str());
...
void TraCIScenarioManager::parseModuleTypes() {
  ...
  std::string moduleDisplayStrings = par("moduleDisplayString")
                                       .stdstringValue();
  ...
  // perform consistency check. for each vehicle type in moduleType there must
  // be a vehicle type in moduleName (and in moduleDisplayString if
  // moduleDisplayString is not empty)
  ...
  if (intersection.size() != typeKeys.size()
    ...
    throw cRuntimeError("keys of mappings of moduleType and moduleName are not "
                        "the same");
  ...
  std::set_intersection(typeKeys.begin(),
                        typeKeys.end(),
                        displayStringKeys.begin(),
                        displayStringKeys.end(),
                        std::back_inserter(intersection));
...
void TraCIScenarioManager::initialize(int stage) {
  ...
  while (std::getline(roiRects_i, rect, ' ')) {
    ...
    rect_i >> y2;
    ASSERT(rect_i);
    roiRects.push_back(std::pair<TraCICoord, TraCICoord>(TraCICoord(x1, y1),
                                                         TraCICoord(x2, y2)));
  }
...
void TraCIScenarioManager::init_traci() {
  {
    std::pair<uint32_t, std::string> version = getCommandInterface()
                                                 ->getVersion();
    ...
    if ((apiVersion == 10) || (apiVersion == 11) || (apiVersion == 13)
        || (apiVersion == 14)) {
      MYDEBUG << "TraCI server \"" << serverVersion << "\" reports API version "
              << apiVersion << endl;
    } else {
      error("TraCI server \"%s\" reports API version %d, which is unsupported. "
            "We recommend using SUMO 0.29.0",
            serverVersion.c_str(),
            apiVersion);
    }
  }

  {
    ...
    TraCIBuffer buf = connection->query(CMD_GET_SIM_VARIABLE,
                                        TraCIBuffer() << static_cast<uint8_t>(
    ...
    if (world
        && ((connection->traci2omnet(netbounds2).x > world->getPgs()->x)
      ...
      MYDEBUG << "WARNING: Playground size (" << world->getPgs()->x << ", "
              ...
              << connection->traci2omnet(netbounds1).y << ")" << endl;
    }
  }

  {
    // subscribe to list of departed and arrived vehicles, as well as simulation
    // time
  ...
  if (msg == executeOneTimestepTrigger) {
    if (simTime() > 1) {
      if (vehicleTypeIds.size() == 0) {
        std::list<std::string> vehTypes = getCommandInterface()
                                            ->getVehicleTypeIds();
        for (std::list<std::string>::const_iterator i = vehTypes.begin();
             i != vehTypes.end();
             ++i) {
          ...
          if (par("useRouteDistributions").boolValue() == true) {
            if (std::count(routeId.begin(), routeId.end(), '#') >= 1) {
              MYDEBUG << "Omitting route " << routeId
                      << " as it seems to be a member of a route distribution "
                         "(found '#' in name)"
                      << std::endl;
              continue;
            }
          }
          MYDEBUG << "Adding " << routeId << " to list of possible routes"
                  << std::endl;
          routeIds.push_back(routeId);
        }
      }
      for (int i = activeVehicleCount + queuedVehicles.size(); i < numVehicles;
           i++) {
        insertNewVehicle();
      }
    }
...
void TraCIScenarioManager::updateModulePosition(cModule *mod,
                                                const Coord &p,
...
void TraCIScenarioManager::addModule(std::string nodeId,
                                     std::string type,
                                     std::string name,
  ...
  // TODO: this trashes the vectsize member of the cModule, although nobody
  // seems to use it
  cModule *mod = nodeType->create(name.c_str(),
                                  parentmod,
                                  nodeVectorIndex,
                                  nodeVectorIndex);
  ...
  if (roiRects.size() > 0) {
    for (std::list<std::pair<TraCICoord, TraCICoord>>::const_iterator i =
      ...
      if ((position.x >= i->first.x) && (position.y >= i->first.y)
          && (position.x <= i->second.x) && (position.y <= i->second.y))
        return true;
    }
  }
  return false;
...
void TraCIScenarioManager::insertVehicles() {
  ...
  for (std::map<int, std::queue<std::string>>::iterator i =
    ...
    while (!i->second.empty()) {
      ...
      suc = getCommandInterface()->addVehicle(veh.str(),
                                              type,
                                              route,
                                              simTime());
      if (!suc) {
  ...
  for (uint8_t j = 0; j < variableNumber_resp; ++j) {
    ...
    if (isokay != RTYPE_OK) {
      ...
      if (isokay == RTYPE_NOTIMPLEMENTED)
        error("TraCI server reported subscribing to variable 0x%2x not "
              "implemented (\"%s\"). Might need newer version.",
              variable1_resp,
              description.c_str());
      error("TraCI server reported error subscribing to variable 0x%2x "
            "(\"%s\").",
            variable1_resp,
            description.c_str());
    }
    ...
    if (variable1_resp == VAR_DEPARTED_VEHICLES_IDS) {
      ...
      for (uint32_t i = 0; i < count; ++i) {
        ...
        // check if this object has been deleted already (e.g. because it was
        // outside the ROI)
      ...
      uint32_t count;
      buf >> count;
      MYDEBUG << "TraCI reports " << count << " vehicles starting to teleport."
              << endl;
      for (uint32_t i = 0; i < count; ++i) {
        std::string idstring;
        buf >> idstring;
      ...
      uint32_t count;
      buf >> count;
      MYDEBUG << "TraCI reports " << count << " vehicles starting to park."
              << endl;
      for (uint32_t i = 0; i < count; ++i) {
        std::string idstring;
        buf >> idstring;
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId,
  ...
  for (uint8_t j = 0; j < variableNumber_resp; ++j) {
    ...
    if (isokay != RTYPE_OK) {
      ...
      if (isSubscribed) {
        if (isokay == RTYPE_NOTIMPLEMENTED)
          error("TraCI server reported subscribing to vehicle variable 0x%2x "
                "not implemented (\"%s\"). Might need newer version.",
                variable1_resp,
                errormsg.c_str());
        error("TraCI server reported error subscribing to vehicle variable "
              "0x%2x (\"%s\").",
              variable1_resp,
              errormsg.c_str());
      }
      ...
      std::set_difference(drivingVehicles.begin(),
                          drivingVehicles.end(),
                          subscribedVehicles.begin(),
                          subscribedVehicles.end(),
                          std::inserter(needSubscribe, needSubscribe.begin()));
      for (std::set<std::string>::const_iterator i = needSubscribe.begin();
           i != needSubscribe.end();
           ++i) {
      ...
      std::set_difference(subscribedVehicles.begin(),
                          subscribedVehicles.end(),
                          drivingVehicles.begin(),
                          drivingVehicles.end(),
                          std::inserter(needUnsubscribe,
                                        needUnsubscribe.begin()));
      for (std::set<std::string>::const_iterator i = needUnsubscribe.begin();
           i != needUnsubscribe.end();
           ++i) {
  ...
  if (!mod) {
    ...
    if (iType == moduleType.end()) {
      ...
      if (iType == moduleType.end())
        throw cRuntimeError("cannot find a module type for vehicle type \"%s\"",
                            vType.c_str());
    }
    mType = iType->second;
    ...
    if (iName == moduleName.end()) {
      ...
      if (iName == moduleName.end())
        throw cRuntimeError("cannot find a module name for vehicle type \"%s\"",
                            vType.c_str());
    }
    mName = iName->second;
    if (moduleDisplayString.size() != 0) {
      ...
      if (iDisplayString == moduleDisplayString.end()) {
        ...
        if (iDisplayString == moduleDisplayString.end())
          throw cRuntimeError("cannot find a module display string for vehicle "
                              "type \"%s\"",
                              vType.c_str());
      }
    ...
    MYDEBUG << "module " << objectId << " moving to " << p.x << "," << p.y
            << endl;
